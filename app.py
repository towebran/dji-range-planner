...
if 'map_key' not in st.session_state: st.session_state.map_key = 0 # FORCING RE-CENTER
...
with st.sidebar:
    ...
    if not st.session_state.dock_confirmed:
        ...
        query = st.text_input("Find Site (Address or Lat, Lon)", placeholder="Acworth, GA")
        ...
        if st.button("📍 Jump to Location"):
            ...
            # THE FIX: Change the map_key to force a hard reload at the new center
            st.session_state.map_key += 1
            st.rerun()
        
        st.info("Now **Click the Map** to refine the Blue Marker. Then click 'Confirm' below.")
...
# --- 4. MAP RENDERING ---
# Pass 'center' and 'zoom' directly into st_folium to override the internal component state
...
for path in st.session_state.vault:
    for seg in path: folium.PolyLine(seg['coords'], color=seg['color'], weight=seg['weight']).add_to(m)
...
out = st_folium(
    m, 
    center=st.session_state.center, # Explicitly tell component where to center
    key=f"map_{st.session_state.map_key}", # Change key to force a fresh component
