...
with st.sidebar:
    ...
    m_dist = st.number_input("Dist (ft)", value=2500, min_value=0)
    # FIXED: Added min_value=0.0 to allow any height input
    m_h = st.number_input("Obs Height AGL (ft)", value=50.0, min_value=0.0)
    ...
    if st.button("🚀 RUN STRATEGIC SCAN"):
        with st.spinner("Executing 16-Direction Scan..."):
            ...
            if res.get('candidates'):
                ...
                try:
                    geolocator = Nominatim(user_agent="m4td_planner_v5")
                ...
                for name, ang in bearings.items():
                    ...
                    for d in range(D_STEP, 19000, D_STEP):
                        ...
                        for m_ob in st.session_state.manual_obs:
                            if m_ob['dir'] == name and abs(m_ob['dist'] - d) < (D_STEP/2 + 200):
...
for o in st.session_state.manual_obs:
    bearings_map = {"N":0, "NNE":22.5, "NE":45, "ENE":67.5, "E":90, "ESE":112.5, "SE":135, "SSE":157.5,
                "S":180, "SSW":202.5, "SW":225, "WSW":247.5, "W":270, "WNW":292.5, "NW":315, "NNW":337.5}
    p = geodesic(feet=o['dist']).destination(st.session_state.center, bearings_map[o['dir']])
    folium.Marker([p.latitude, p.longitude], tooltip=f"MANUAL OBS: {o['agl']}ft AGL", icon=folium.Icon(color='orange', icon='warning')).add_to(m)
