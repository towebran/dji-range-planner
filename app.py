...
TX_EIRP = 33.0        
RX_SENSITIVITY = -95.0 
MARGIN_HD = -82.0      # Green threshold
MARGIN_DEGRADED = -90.0 # Orange threshold
EARTH_K = 1.333        
...
def get_signal_color(dist_ft, h_tx, h_rx, terrain_obs_msl, manual_hits):
    """Calculates decaying signal strength (RSSI)."""
    dist_mi = dist_ft / 5280.0
    dist_km = dist_ft / 3280.84
    curv_drop = (dist_mi**2) / (1.5 * EARTH_K)
    
    # Base Path Loss (2.4GHz)
    fspl = 20 * math.log10(max(0.01, dist_km)) + 20 * math.log10(2.4) + 92.45
    rssi = TX_EIRP + 3.0 - fspl
    
    # Check Obstruction Penalties
    for m in manual_hits:
        if m['dist'] < dist_ft:
            beam_at_obs = h_tx + (h_rx - h_tx) * (m['dist'] / dist_ft)
            clearance = beam_at_obs - (m['msl'] + curv_drop)
            if clearance < 0:
                # Trees attenuate, Solids block much harder
                rssi -= (12.0 if m['type'] == "Tree" else 30.0)
    
    # Global Clutter Check
    if terrain_obs_msl > h_rx: rssi -= 20.0

    if rssi > MARGIN_HD: return "#00FF00", 5    # Green: Solid
    if rssi > MARGIN_DEGRADED: return "#FFA500", 3 # Orange: Degraded
    return "#FF0000", 2                        # Red: Lost
...
with st.sidebar:
    ...
    if not st.session_state.dock_confirmed:
        ...
        query = st.text_input("Enter Address or Lat/Lon", "Acworth, GA")
        ...
        clutter_h = st.slider("Global Clutter (ft)", 0, 100, 60)
        ...
        if st.button("🚀 RUN 8-DIRECTION SCAN"):
            with st.spinner("Analyzing..."):
                ...
                for ang in [0, 45, 90, 135, 180, 225, 270, 315]:
                    ...
                    for d in range(1300, 19500, 1300):
...
for path in st.session_state.vault:
    ...
    # Add Distance Label at the end of the line
    last_seg = path[-1]
    miles = round(last_seg['dist'] / 5280, 2)
    folium.Marker(last_seg['coords'][1], icon=DivIcon(html=f'<div style="color:white; background:rgba(0,0,0,0.6); padding:2px; font-size:10px; border-radius:3px;">{miles}mi</div>')).add_to(m)
...
out = st_folium(m, width=1000, height=600, center=st.session_state.center, key=st.session_state.map_key)
...
if out and out.get("last_clicked") and st.session_state.dock_confirmed:
    ...
    st.session_state.manual_obs.append({"id": new_id, "coords": [lat, lon], "msl": get_elev_msl(lat, lon) + 60, "type": "Tree", "dist": int(geodesic(st.session_state.center, (lat, lon)).feet)})
    st.rerun()
